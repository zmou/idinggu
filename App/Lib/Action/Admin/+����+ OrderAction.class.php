            ...
            if($this->_post()){
                ...
                if($arr['order_status']==3){
					...
					if(!empty($parent_info['parent_1'])){
						...
						$score=$data['total_fee']*($percent['second_percent']/100);
						//file_put_contents('log.html',$data['total_fee'].'<br>score:'.$score.'<br/>second_percent'.$percent['second_percent'],FILE_APPEND);
					...
					if(!empty($parent_info['parent_2'])){
						...
						$score=$data['total_fee']*($percent['first_percent']/100);
						if($info['user_level']==2){
