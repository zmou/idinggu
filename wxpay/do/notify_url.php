...
if($result_code=='SUCCESS'&&$return_code=='SUCCESS'){
	...
	if(count($trade_no_arr)>1)
	{
		...
		if($friend_pay['payed_money'] >= $order_info['total_fee'])
		{
			...
			{
				$sms_content = "【叮咕】亲，订单来啦！待配送商品：".$order_info['order_title'].";送货地址：".$order_info['build']."-".$order_info['address']." 联系人姓名：".$user_info['name']." 电话：".$order_info['mobile'].";赶紧查看您的待配送订单详请，5分钟内惊呆您的小伙伴吧～";
				//$sms_content = "【叮咕】亲，又来订单啦！赶紧查看您的待配送订单详请，5分钟内惊呆您的小伙伴吧～";
				send_sms($shop_keeper['mobile'],$sms_content);
			}

			//add 10 jifen
			$sql = "update twotree_wechat_user set jifen=jifen+10";
			$db->query($sql);
		}
	...
	if($order_info['role_id']==1 && $order_info['order_style']==1)
	{
		...
		file_put_contents('pay'.date('YmdHis').'-test_sms-u'.$shop_keeper['id'].'.txt',$shop_keeper['mobile'].'\n');
