...
<div class="coin_title2">
	...
	<?php if(is_array($cat[goods_list])): $i = 0; $__LIST__ = $cat[goods_list];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><dl class="goods_list">
		<dt>
			<a href="<?php echo U('product_show',array('id'=>$item['id']));?>"><img src="<?php echo ($item["spic"]); ?>" border="0" /></a> </dt>
		<dd>
		<b><a href="<?php echo U('product_show',array('id'=>$item['id']));?>"><?php echo ($item["name"]); ?></a></b>
		<b style="color:#9F9F9F">库存：<font <?php if(($item['store_num']) == "0"): ?>style="color:red;"<?php endif; ?>><?php echo (($item["store_num"])?($item["store_num"]):0); ?></font></b>
		<b>
			<?php if(($shop['shop_status']) == "1"): if(($user_info['role_id']) == "1"): ?><span style="padding-right:30px;"><img class="btn-cart" id="add_cart_<?php echo ($item["id"]); ?>" src="__PUBLIC__/images/product_r8_c11.png" goods_id="<?php echo ($item["id"]); ?>" goods_price="<?php echo ($item["price"]); ?>" store_num="<?php echo ($item["store_num"]); ?>" /></span><?php endif; ?>
			<?php else: ?>
			<span style="color:#FF0000">休息中</span><?php endif; ?>
