...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><dl class="cart_list">
	...
	<b>
		<p class="p_num"> 
		<span class="sy_minus num_sub" id="sy_minus_gid1" goods_id="<?php echo ($item["goods_id"]); ?>" package_num="<?php echo ($item["package_num"]); ?>">-</span>  
		<input class="sy_num" id="sy_num_gid1" readonly="readonly" type="text" value="<?php echo ($item["goods_nums"]); ?>" />  
		<span class="sy_plus num_add" id="sy_plus_gid1" goods_id="<?php echo ($item["goods_id"]); ?>" package_num="<?php echo ($item["package_num"]); ?>">+</span> 
		</p>
...
<div class="footer_menu">
	<ul>
		...
		<li>
		<a href="<?php if(($user_info["role_id"]) == "1"): echo U('Index/cart'); else: echo U('Ucenter/replenishment'); endif; ?>"  <?php if((MODULE_NAME == 'Index') AND (in_array(ACTION_NAME,array('cart','cart2')))): ?>class="act"<?php endif; ?>>
