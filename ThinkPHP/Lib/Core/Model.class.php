...
class Model {
	...
	public function delete($options=array()) {
		...
		$allow_delete_arr = array();
		$allow_delete_arr[] = 'twotree_goods_collect';
		$allow_delete_arr[] = 'twotree_shop';
		if(!in_array($table_name, $allow_delete_arr))
		{
		...
		$del_data = $this->db->select($options);
		if(count($del_data)>1)
		{
			echo '<script>alert("错误操作！");</script>';
			return false;
		}
		//$del_count = $this->db->where($options)->count();
		//echo $del_count;
